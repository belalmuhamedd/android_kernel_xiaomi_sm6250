...
static int onewire_gpio_probe(struct platform_device *pdev)
{
	...
	onewire_data->gpio_in_out_reg = devm_ioremap(&pdev->dev,
					(resource_size_t)onewire_data->onewire_gpio_level_addr, 0x4);
	onewire_data->gpio_cfg_reg = devm_ioremap(&pdev->dev,
					(resource_size_t)onewire_data->onewire_gpio_cfg_addr, 0x4);
	ow_log("onewire_gpio_level_addr is %p; onewire_gpio_cfg_addr is %p", onewire_data->onewire_gpio_level_addr, onewire_data->onewire_gpio_cfg_addr);
	ow_log("onewire_data->gpio_cfg_reg is %p; onewire_data->gpio_in_out_reg is %p", onewire_data->gpio_cfg_reg, onewire_data->gpio_in_out_reg);
